...
def load_vectordb(docs):
    return create_vectordb(docs)


@st.cache_data
def process_video(video_link):
    loader_chain = RunnableLambda(get_transcript) | RunnableLambda(split_text)
    return loader_chain.invoke(video_link)
...
After all the question provide the answers:
Answers:
